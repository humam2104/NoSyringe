class DummySyringeHealingBase extends KFWeap_HealerBase;
...
function GetSyringeConfig(){
		DummySyringeStandAloneHealAmount = 0;
		DummySyringeOthersHealAmount = 0;
		DummySyringeHealSelfRechargeSeconds = 15;
		DummySyringeHealOthersRechargeSeconds = 7.5;		
		`log("*** Dummy Syringe Parameters are set successfuly! ***");
}
...
simulated function CustomFire() // Redeclared parent function to replace variables
{
	...
	if( Role == ROLE_Authority )
	{
		...
		{
			HealAmount = DummySyringeOthersHealAmount;															 
			HealTarget.HealDamage( HealAmount, Instigator.Controller, InstantHitDamageTypes[CurrentFireMode]);
			HealRechargeTime = DummySyringeHealOthersRechargeSeconds;
		}
		// Healing Self
		else if( CurrentFireMode == ALTFIRE_FIREMODE )
		{
			...
			{
				HealAmount = DummySyringeStandAloneHealAmount; 														// Replaced "StandAloneHealAmount" for solo with my own variable
			}
			else
			{
				HealAmount = DummySyringeStandAloneHealAmount; 														// Replaced "StandAloneHealAmount" for solo with my own variable
			...
			HealRechargeTime = DummySyringeHealSelfRechargeSeconds; 												// Replaced "HealSelfRechargeSeconds"
		}
	}
}
